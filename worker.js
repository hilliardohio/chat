...
async function searchPrograms(env, query) {
  try {
    ...
    const scoreOf = p => {
      const name = (p.name || '').toLowerCase(), desc = (p.description || '').toLowerCase(), c = (p.category || '').toLowerCase();
      const secs = p.sections || [];
      const secText = secs.map(s => [s.title, s.days, s.location].join(' ').toLowerCase()).join(' ');
      let s = 0;
      for (const t of literal) { if (name.includes(t)) s += 6; if (secText.includes(t)) s += 4; if (c.includes(t)) s += 2; if (desc.includes(t)) s += 1; }
      for (const t of broad) { if (name.includes(t)) s += 2; if (c.includes(t)) s += 2; if (secText.includes(t)) s += 1; if (desc.includes(t)) s += 0.5; }
      if (age != null && s > 0) {
    ...
    const ranked = (cat.programs || []).map(p => ({ p, s: scoreOf(p) })).filter(x => x.s > 0).sort((a, b) => b.s - a.s).slice(0, 8);
